...
LinkedAlex는 특정 논문 또는 연구 주제 주변의 인용 맥락과 연구 흐름을 탐색하는 데 사용할 수 있습니다.

1. 인용 수가 많은 방법론 연구 A를 주제로 분석하여, 해당 방법론의 수학적 기초가 되는 선행연구와 응용 방향을 보여주는 후속연구를 조사합니다.
2. 주요 선행연구를 리버스엔지니어링하거나 개선하기 위해 분석하고, 후속 연구를 추적해 연구가 어떻게 발전하는지 파악합니다.
3. 관심 분야의 연구가 어느 저널에 주로 발행되는지 파악합니다.
4. 관심 있는 연구가 어떤 키워드로 설명되는지 파악한 뒤, 해당 키워드를 **Keyword Search** 섹션에서 AND/OR 조건으로 검색하여 그래프에 잡히지 않은 추가 논문을 탐색하고, 해당 논문을 기준으로 그래프를 재생성할 수 있습니다.
5. CSV 또는 JSON으로 export한 논문 정보 테이블을 LLM의 입력으로 사용하여, 더 자세한 질문을 하고 할루시네이션이 억제된 응답을 얻을 수 있습니다.
   - 논문 A가 왜 B-Z를 인용했는지에 대한 근거를 질문할 수 있습니다.
   - DOI를 바탕으로 원문을 수집한 뒤, 원문을 입력하여 공통적으로 사용된 방법론 등 abstract에 없는 자세한 정보를 질문할 수 있습니다.
